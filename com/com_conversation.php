...
$cid = token_id();
$_conv = $db->get_row("select * from ".DB_PREFIX."conversations where conf_id = '".$cid."' and user_id='".user_id()."'");
...
if($_conv) {
    ...
    $anothers = $db->get_results("select * from ".DB_PREFIX."conversations where conf_id = '".$cid."' and user_id <> '".user_id()."'");
    
    $anotherUsers = [];

    foreach ($anothers as $another)
    {
        array_push($anotherUsers, $another);
    }

    $other = $anotherUsers[0]->user_id;
