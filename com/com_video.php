...
//return only comments ;)
    ...
    $video = $db->get_row("SELECT " . DB_PREFIX . "videos.*, " . DB_PREFIX . "channels.cat_name as channel_name ," . DB_PREFIX . "users.avatar, " . DB_PREFIX . "users.name as owner, " . DB_PREFIX . "users.group_id, " . DB_PREFIX . "users.avatar FROM " . DB_PREFIX . "videos 
...
//Premium video
            $embedvideo = '<div class="vprocessing" style="background-image: url(' . "'$video->thumb'" . ');background-size: contain;
background-repeat: no-repeat;">
<div class="blured-block"><div class="vpre">' . _lang("This video is for premium users only!") . '</div> 
<div class="vex">' . _lang("Become a premium user for as low as ") . ' ' . get_option("monthlycurrency", "USD") . ' ' . get_option("monthlyprice", "1") . '
<div class="full text-center mtop20"><a class="btn btn-primary" style="color:white !important; font-weight:bold" href="' . site_url() . 'payment">' . _lang("Upgrade") . '</a></div>
</div>
</div>
</div>';
