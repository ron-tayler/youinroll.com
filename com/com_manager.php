...
// Canonical url
    ...
    if (_get('sk')) {
        $canonical = site_url() . me . '?sk=' . _get('sk');
    }
    ...
    if (isset($_POST['edited-video']) && !is_null(intval($_POST['edited-video']))) {
        ...
        if (isset($_POST['isjitsi']) && not_empty($_POST['isjitsi'])) {
            $db->query("UPDATE  " . DB_PREFIX . "videos SET isjitsi='" . toDb($_POST['isjitsi']) . "' WHERE user_id= '" . user_id() . "' and id = '" . intval($_POST['edited-video']) . "'");
        }
...
//$uploader -> resizeImage(200, 200, 'crop');
        ...
        $db->query("INSERT INTO " . DB_PREFIX . "playlists (`owner`, `title`, `picture`, `description`, `price`, `card_number`) VALUES ('" . user_id() . "','" . toDb($_POST['play-name']) . "', '" . toDb($picture) . "' , '" . toDb($_POST['play-desc']) . "', '" . toDb($_POST['play-price']) . "', '" . toDb($_POST['play-card']) . "')");
