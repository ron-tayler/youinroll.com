...
if (isset($_POST['vtoken'])) {
    ...
    $doit = $db->get_row(sprintf("SELECT id from %svideos where token = '%s'", DB_PREFIX, $tok));
    if ($doit) {
        if (get_option('ffa', '0') != 1) {
            ...
            if (isset($_GET['for']) && $_GET['for'] === 'landing') {
                ...
                $db->query(sprintf(
                    "UPDATE %svideos SET privacy='%s',pub='%s',title='%s',description='%s',category='%s',tags='%s',nsfw='%s' WHERE user_id='%s' and id ='%s'",
                    DB_PREFIX,
                    intval(_post('priv')),
                    intval(get_option('videos-initial')),
                    toDb(_post('title')),
                    toDb(_post('description')),
                    toDb(intval(_post('categ'))),
                    toDb(_post('tags')),
                    intval(_post('nsfw')),
                    user_id(),
                    intval($doit->id)
