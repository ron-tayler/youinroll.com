...
global $ps;
	...
	//return admin_url('conferences').'&sort='.$sorter.'&p=1';
}
...
if(isset($_POST['checkRow'])) {
	foreach ($_POST['checkRow'] as $del) {
		...
		break;
		case "feature":
			//$db->query("UPDATE ".DB_PREFIX."conferences set featured = '1' where id='".intval($del)."'");
		break;
		case "premium":
...
$order = "ORDER BY ".DB_PREFIX."conferences.id desc";
$where = "";
$sortA = array();
if(isset($_GET['sort'])) {
...
$count = $db->get_row("Select count(*) as nr from ".DB_PREFIX."conferences");
$videos = $db->get_results("select * from ".DB_PREFIX."conferences $order ".this_limit()."");
//$db->debug();
?>
<div class="row">
...
if($videos) {
$sort=	implode(";",$sortA );
$ps = admin_url('conferences').'&sort='.$sort.'&p=';
$a = new pagination;	
...
?>
<form class="form-horizontal styled" action="<?php echo admin_url('conferences');?>&p=<?php echo this_page();?>" enctype="multipart/form-data" method="post">
...
<ul class="list-group">
	...
	<img class="row-image" src="<?php echo($video->cover); ?>">
	</span>
	</div>
	<div class="inline-block right20 img-txt">
	<h4><a target="_blank" href="<?php echo conference_url($video->name);?>"><?php echo _html($video->name); ?> </a></h4>
	<p><?php echo($video->description); ?></p>
	<div class="img-det-text">
	<i class="material-icons">&#xE192;</i> <?php echo time_ago($video->created_at); ?>
	</div>
	
	</div>
	</div>
	 <div class="btn-group btn-group-vertical pull-right">
		<a class="btn btn-sm btn-outline btn-default confirm" href="<?php echo admin_url('conferences');?>&p=<?php echo this_page();?>&delete-video=<?php echo $video->id;?>">
		<i class="material-icons mright10"> delete </i> unlist
		</a>
		<a class="btn btn-sm btn-raised btn-primary" href="<?php echo admin_url('edit-conference');?>&vid=<?php echo $video->id;?>">
