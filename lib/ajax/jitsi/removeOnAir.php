...
if($streamInfo !== null)
{
    $userInfo = $db->get_row("SELECT id,avatar,name,onAir FROM ".DB_PREFIX."users where id = ".toDb(user_id())." limit  0,1");
    $userInfo->isAuthor = ((int)$streamInfo->moderator_id === user_id());

    if($userInfo->isAuthor)
    {
        $db->query('UPDATE '.DB_PREFIX.'users SET onAir = false WHERE id = '.toDb(user_id()));
    }
}
?>