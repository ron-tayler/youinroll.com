...
{
    global $db;
    $gr = $db->get_row("SELECT id as prem from " . DB_PREFIX . "users_groups WHERE ispremium=1 and id > 2");
    if ($gr) {
        return $gr = $gr->prem;
    }
    return false;
...
function premium_level($level, $channelId)
{
    ...
    $premium = $db->get_row('SELECT id from ' . DB_PREFIX . 'user_subscriptions WHERE user_id = ' . toDb(user_id()) . ' AND channel_id = ' . toDb($channelId) . ' AND level = ' . toDb($level));
...
//avoid not set issues
        ...
        if (!isset($userData['group_id']) || nullval($userData['group_id'])) {
            $userData['group_id'] = '4';
        }
        if (!isset($userData['bio']) || nullval($userData['bio'])) {
            $userData['bio'] = '';
        }
        if (!isset($userData['chatRoom']) || nullval($userData['chatRoom'])) {
            $userData['chatRoom'] = '';
        }
//insert to db
    ...
    public static function loginbymail($mail, $pass = null)
    {
        ...
        if ($result && $result->id) {
            ...
            $chatRoom = $db->get_row("SELECT chatRoom FROM " . DB_PREFIX . "users WHERE id = '" . $result->id . "'");

            if ($chatRoom->chatRoom === null) {
                $jitsiLogin = str_replace('@', '-dmnSymb-', $mail);
                ...
                $tt = "UPDATE " . DB_PREFIX . "users SET haveJitsi = '1', jitsiLogin = '" . $jitsiLogin . "' WHERE id = '" . $result->id . "'";
