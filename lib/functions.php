...
function subscribe_box($user, $btnc = '', $counter = false, $type = 'default'){
	global $db;
	$ktool ='';
	...
	if (!is_user()) {
		...
		if($type === 'modal') {
			...
			echo '<a class="'.$btnc.'" href="/login"><i class="icon icon-plus"></i>'._lang('Follow').' '.$ktool.'</a>';

		} else 
		{
			echo '<a class="'.$btnc.'" href="/login"><i class="icon icon-plus"></i>'._lang('Subscribe').' '.$ktool.'</a>';
		}
	} elseif ($user <> user_id()) {
		//If it's not you
		...
		if(is_video()) {
			...
			//It's you
			$btnc = "btn btn-default subscriber";
			echo '<a href="'.site_url().'dashboard/?sk=edit" style="background-color:#3598dc;" class="'.$btnc.'"><i class="icon icon-cogs"></i>'._lang('Settings').'</a>';
		}
	}
