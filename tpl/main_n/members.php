...
echo _lang('Sorry but there are no results.');
 ...
 if(this_page() < 2) {
	...
	$topusers = $db->get_results("select name,group_id,avatar,id from ".DB_PREFIX."users order by views DESC ".this_limit()."");
		if($topusers) {
			echo '<h1 class="loop-heading"><span>'._lang("Самые просматриваемые").'</span></h1>';
	...
	<li class="subitem">
			<a class="block text-center" data-id="'.$user->id.'" title="'.$full_title.'" href="'.$url.'">
						<img class="img-circle cartistic" src="'.thumb_fix($user->avatar, true, 100, 100).'" data-name="'.$full_title.'" alt="logo" />
			</a>
			<span class="btn btn-block member-name">'._html($title).' '.$grcreative.'</span>
			<div class="block">';
	...
	$actusers = $db->get_results("select o.name,o.id,o.group_id, o.avatar, c.user, count(c.user) as actives from ".DB_PREFIX."activity c left join ".DB_PREFIX."users o on c.user = o.id group by c.user order by actives desc
	 ".this_limit()."");
		if($actusers) {
			echo '<h1 class="loop-heading"><span>'._lang("Наиболее активные").'</span></h1>';
		echo '<div class="fake-padding black-slider"> <ul id="carousel" class="owl-carousel">'; 
	...
	<li class="subitem">
			<a class="block text-center" data-id="'.$user->id.'" title="'.$full_title.'" href="'.$url.'">
						<img class="img-circle cartistic" src="'.thumb_fix($user->avatar, true, 100, 100).'" data-name="'.$full_title.'" style="width:100px; height:100px" alt="logo" />
			</a>
			<span class="btn btn-block member-name">'.$full_title.' '.$grcreative.'</span>
			<div class="block">';
			subscribe_box($user->id);
			echo '</div>
	</li>';
