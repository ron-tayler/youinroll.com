<?php
/**
 * @file page user profile
 * @var ezSQL_mysqli $db
 * @var ezSQL_mysqli $cachedb
 */
...
<!--                            </div>-->
                    ...
                    <div class="btn-group">
                        <? if ($_GET['rtf'] == 'debug') { ?>
                            <? if ($profile->id == user_id()) { ?>
                                <div class="btn-group">
                                    <a href="/me?sk=playlists"
                                       style="background-color:#fe2c55; border-color: #fe2c55;"
                                       class="btn btn-coral subscriber">
                                        <i class="icon icon-cogs"></i>
                                        <span>Менеджер курсов</span>
                                    </a>
                                    <a href="/dashboard/?sk=edit"
                                       style="background-color:#3598dc;border-color: #3598dc;"
                                       class="btn btn-default subscriber">
                                        <i class="icon icon-cogs"></i>
                            ...
                            <? } else { ?>
                                <div class="btn-group">
                                    <?php
                                    $sub_text = 'Подписаться';
                                    $title_sub_text = 'Нажмите чтобы подписаться';
                                    /* TODO Требуется сделать систему платных подписок
                                    if (is_user()) {
                                        $check = $db->get_var("SELECT count(*) from vibe_users_friends where uid ='$user' and fid='".user_id()."'");
                                        if($check){
                                            $sub_text = 'Отписаться';
                                            $title_sub_text = 'Нажмите чтобы отписаться';
                                        }
                                    }*/ ?>
                                    <a id="modal-<?= $user ?>"
                                       data-next="<?= $sub_text ?>"
                                       class="btn btn-coral social-google-plus subscriber pv_tip"
                                       href="javascript:SubscribeModal(<?= $user ?>)"
                                       title="<?= $title_sub_text ?>">
                                        <span class="follow-text"><?= $sub_text ?></span>
                                    </a>
                                </div>
            ...
            <div id="profile-nav" class="red-nav">
                <ul>
                    ...
                    <?php
                    $is_images = $db->get_var('SELECT count(*) FROM ' . DB_PREFIX . 'images WHERE user_id = ' . toDb($profile->id) . ' AND pub=1 AND private=0 LIMIT 0,1');
                    ?>
                    <? if (get_option('imagesmenu', '1') == 1 and (bool)$is_images) { ?>
                        <li class="<?= aTab("images"); ?>">
                            <a href="<?= $canonical; ?>?sk=images"><?= _lang("Images"); ?></a>
                        </li>
                    <? } ?>
                    <? if (get_option('musicmenu', '1') == 1) { ?>
                        <li class="<?= aTab("music"); ?>"><a
                                    href="<?= $canonical; ?>?sk=music"><?= _lang("Music"); ?></a></li>
                    <? } ?>
                    <? if (in_array($profile->group_id, ['1', '11', '12', '14', '15'], true)) { ?>
                        <li class="<?= aTab("timeline"); ?>">
                            <a href="<?= $canonical; ?>?sk=timeline"><?= _lang("Расписание"); ?></a>
                        </li>
                    <? } ?>
                </ul>
...
<script>
    ...
    var profileInfoSticky = profileInfo.offsetTop;

    // Add the sticky class to the header when you reach its scroll position. Remove "sticky" when you leave the scroll position
    function myFunction() {
        if (window.pageYOffset > profileInfoSticky) {
