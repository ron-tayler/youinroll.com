...
jQuery(function($) {
    ...
    if ($(window).width() < 768) {
        ...
        if (localStorage.getItem('menuState') !== 'open' && localStorage.getItem('menuState') !== null) {
...
$(document).ready(function (key, value) {
    ...
    $('#mobile-media-description').readmore({
        speed: 75,
        collapsedHeight: 0,
        lessLink: '',
        moreLink: ''
    ...
    if ($(window).width() < 990) {
        ...
        $('.vibe-interactions > h1').html(hinit + '<a href="javascript:showDescription()" data-active="0" class="description-pull pull-right"><i class="material-icons mpulldown"></i></a>');

        $('#media-description').readmore('destroy');
        $('#media-description').hide();
...
function showDescription() {
    ...
    if ($('.description-pull').data('active') === 1) {
        $('#mobile-media-description').readmore('toggle')
        $('.description-pull').css('transform', 'rotate(0deg)')
        $('.description-pull').css('transitionDuration', '1s')
        $('.description-pull').data('active', 0)
    } else {
        $('#mobile-media-description').readmore('toggle')
        $('.description-pull').css('transform', 'rotate(180deg)')
        $('.description-pull').css('transitionDuration', '1s')
        $('.description-pull').data('active', 1)
    }
...
function iLikeThis(vid) {
    ...
    if ($('#i-dislike-it').hasClass('done-dislike') || $('#i-dislike-it').hasClass('isLiked')) {
        let likesNrx = parseInt($('#i-dislike-it > span').text());

        $('#i-dislike-it > span').html(likesNrx - 1);
        $('#i-dislike-it').removeClass('done-dislike');
        $('#i-dislike-it').removeClass('isLiked');
    }

    if ($('#i-like-it').hasClass('done-like') ||
        $('#i-like-it').hasClass('isLiked')
    ) {
        RemoveLike(vid);
    } else {
...
function iHateThis(vid) {
    //console.log(a);
    let likesNr = $('#i-dislike-it > span').text();

    likesNr = parseInt(likesNr);

    if ($('#i-like-it').hasClass('done-like') || $('#i-like-it').hasClass('isLiked')) {

        let likesNrx = parseInt($('#i-like-it > span').text());

        $('#i-like-it > span').html(likesNrx - 1);
        $('#i-like-it').removeClass('done-like');
...
function Padd(vid, pid) {
    $.post(
        ...
        function(data) {
            var a = JSON.parse(data);
            $.notify(a.title + ' ' + a.text);
            if ($('a[data-id="' + pid + '"]').html()) {
                $('a[data-id="' + pid + '"]').remove();
            }
            if ($('#video-' + vid).html()) {
                $('a.laterit').remove();
            }
...
function Subscribe(user, type, element) {
    $.post(
...
function SubscribeModal(user, type) {
