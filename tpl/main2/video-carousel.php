...
<div class="video-thumb">
		<a class="clip-link" data-id="'.$video->id.'" title="'.htmlspecialchars(str_replace("|","&#124;",$full_title), ENT_QUOTES).'" href="'.$url.'">
			<span class="clip">
				<img src="'.thumb_fix($video->thumb, true, get_option('thumb-width'), get_option('thumb-height')).'" alt="'.htmlspecialchars($full_title, ENT_QUOTES).'" /><span class="vertical-align"></span>
			</span>
...
<div class="video-data video-data-carousel">
	<div class="chanel-avatar"><img src="'.$video->avatar.'" alt="logo" />
	</div>
...
// echo '<li>'.number_format($video->views).' '.$views_text.'</li>';
echo '<li><img src="/tpl/main/images/eye.svg" class="eye" alt="icon" /> '.number_format($video->views).'&nbsp;|&nbsp;';
if(isset($video->date)) { 
