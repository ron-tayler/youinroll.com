...
function addEMComment(oid,toid){
    if($('textarea#addEmComment_'+toid).val()){
        ...
        $.post(
            ...
            function(data){
			   $('#emContent_'+oid+ '-'+toid+' li:first').after('<li id="comment-'+data.id+'" class="left"><img class="avatar" src="'+data.image+'" /><div class="message"><span class="arrow"> </span><a class="name" href="'+data.url+'">'+data.name+'бббббббб</a> <span class="date-time"> '+data.date+' </span> <div class="body">'+data.text+'</div> </div></li>');
